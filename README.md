...
8. **🎭 模擬示範版（Mock）**：未登入都可以演示成個系統 — 參考 event 系統做法：登入頁／使用地區頁有「🎭 模擬示範版」入口（或直接分享 `?demo=1` 連結）→ 一鍵以 區總監／副區總監／ADC／區職員 示範身份進入，每頁頂部橫額可以隨時轉身份（睇權限分別，例如 ADC 睇唔到獎勵提名）、↺ 重設示範資料、✕ 離開示範版。**100% 本地沙盒**：示範資料（`lib/demo/seed.ts`）內建 18 類獎項名冊／9 個旅探訪／借場借物資／消息（含軟刪除示範行）／房間日曆等，全部改動只存瀏覽器 sessionStorage，唔會打去任何後端（`lib/api.ts` 三個出口 callGet／callPost／callExternal 全部攔截，`lib/demo/engine.ts` 模擬後台語義包括權限閘、一日一旅一次、死線儲存、軟刪除）。測試：`node --experimental-strip-types scripts/test-demo-engine.ts`（20 項）＋`scripts/test-demo-wiring.ts`（7 項）。
