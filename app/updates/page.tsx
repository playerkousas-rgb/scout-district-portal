  ...
  return (
    <>
      ...
      <div className="info-card">
        <h3>v4.8.0 — 🏕 旅團探訪（撳一下就登記 · 季度報告）</h3>
        <ul>
          <li>🗺 <b>探訪登記</b>：全區 28 個旅一格格排開，<b>未探過紅框、探過綠框</b>；撳一下嗰格，日期預設今日、幹部預設你自己，儲存就完</li>
          ...
          <li>⚙️ <b>旅團名單</b>：預設已經入咗港島地域官網嘅筲箕灣區 28 個旅（連主辦機構、五個支部團數），可以自己改或者貼上；改完「活動知會」「聯結簿」嘅旅號清單一齊更新</li>
          <li>⚠️ 要換新 <code>Code.gs</code>（v4.8.0）→ <code>setupSheets()</code>（會建立 <code>Units</code>／<code>Visits</code> 兩張表）→ 重新部署</li>
        </ul>
      </div>
      <div className="info-card">
