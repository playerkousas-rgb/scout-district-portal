...
| 4b. 開班登記 | ADC（管理系統 `/training`） | 貼上 `/exec`＋API Key（兩行一次過貼都識分）＋資料夾 ID＋區網通告 PDF 連結 →「📥 由訓練班 Sheet 讀取」＋「📥 由通告網址讀取」（伺服器抓 PDF 抽字；PDF 或帖文頁連結都得，帖文頁會自動跟去 PDF 正本＋讀埋標籤；通告名／收費／資助／名額／截止／對象／節次／場地／聯絡／班領導人／服裝／備註／報名辦法／費用全文／徽章／支部自動填）→「🔍 自動填好嘅資料」自動展開檢查 → 儲存（啟用）→ 即時掛上成員系統（內置報名表，唔用 Google Form）。**課程代碼唔使填**：留空由後台自動編（`cl_…`） |
