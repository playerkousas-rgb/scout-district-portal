...
check('舊 CourseLinks（v4.15 欄位）→ ensureSheetColumns_ 補 5 欄喺最尾，舊資料原封不動', () => {
  ...
  assert.strictEqual(added.join(','), 'leader,uniform,remarks,signupText,feeNote');
  const headers = oldSheet._data[0];
  // 補欄按表尾追加（舊表 sheetId/setupJson 已喺度，所以 5 個新欄排最尾）
  assert.strictEqual(headers.slice(-5).join(','), 'leader,uniform,remarks,signupText,feeNote');
  // 舊資料仲喺度
  const rows = ctx.readSheet_('CourseLinks');
